...
import { useGetNewsUpdatesQuery } from "../../app/features/news-updates/newsUpdatesApi";
...
const NewsUpdates = ({
  ...
  const { data, isLoading } = useGetNewsUpdatesQuery();
  const [deleteNewsUpdates, { isLoading: isDeleteLoading }] =
    useDeleteTagMutation();
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const description = row.getValue("description") || "N/A";
        return <p className="text-wrap">{description}</p>;
      },
      enableSorting: false,
    },
    {
      ...
      cell: ({ row }) => {
        const category = row.getValue("category")?.name || "N/A";
        return <p>{category}</p>;
      },
      enableSorting: false,
    },
    ...
    },
    {
      accessorKey: "author",
      header: ({ column }) => (
        <DataTableColumnHeaderCommon column={column} title="Author" />
      ),
      cell: ({ row }) => {
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeaderCommon column={column} title="Date Added" />
      ),
      cell: ({ row }) => {
    ...
    <section className="users-section">
          ...
          <>
            ...
            <div className="flex items-center gap-2 justify-between pt-4">
              ...
              <Button
                variant="destructive"
                onClick={async () => {
                  await deleteNewsUpdates(selectedNewsUpdates?.id);
      ...
      {openDeactivateDialog && (
        ...
        >
          <>
            <p>
              {selectedNewsUpdates?.status === APP_CONSTANTS.DEACTIVATED_STATUS
            ...
            <div className="flex items-center gap-2 justify-between pt-4">
              ...
              <Button
                variant={
                  selectedNewsUpdates?.status === APP_CONSTANTS.INACTIVE_STATUS
                    ? "activate"
                    : "destructive"
                }
                onClick={async () => {
                  await updateTag({
                    id: selectedNewsUpdates?.id,
                    status:
                      selectedNewsUpdates?.status ===
