              ...
              >
                <PopoverTrigger asChild className="">
                  ...
                  >
                    ...
                    {initialValue !== null ? (
                      <TypographyP variant="">{initialValue?.name}</TypographyP>
                    ) : (
                      (Array.isArray(items) &&
                        items?.find((item) => {
                          return String(item?.id) === String(field.value);
                        })?.name) || (
                        <TypographyP variant="muted">{placeholder}</TypographyP>
                      )
                    )}
                    <ChevronDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                  </Button>
...
export function ImagePicker({ control, name, label }) {
  const { watch } = useFormContext();
  const [previewImage, setPreviewImage] = useState(watch(name) || null);
  ...
  const handleDrop = (e) => {
    ...
    if (file && file.type.startsWith("image/")) {
      ...
      const changeEvent = { target: { files: fileList } };
      control._formValues[name] = file;
    }
  };
